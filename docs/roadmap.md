...
- The portable plugin source (`/src/`) — verbatim copy from where the code was incubated. Plain ES modules, plain PHP, no build step, no React.
...
## v0.2.x — public launch

The public launch milestone: the plugin is announced to the WordPress community and the host-adapter API is declared stable.
...
- Filter alias deprecation cycle: the `wp_admin_sidebar_*` filter names are canonical; the legacy aliased names are deprecated through this milestone.
...
**Exit criteria:** the plugin has run in production for ≥ 1 week with no open regressions; Make/Core post live; Reddit post live; community feedback channels active.

## v0.3.x — wp.org submission + rollout hardening
...
- Drop the legacy filter aliases. Hosts who haven't migrated by this point break loudly; the migration is a one-line search-and-replace.
- WP-CLI commands: `wp admin-sidebar enable/disable/reset --user=N`.

**Exit criteria:** plugin is installable from wordpress.org/plugins; no open critical issues; first usability-test results in (positive or with a clear list of follow-ups).
...
**Exit criteria:** plugin has had a sustained period of stable rollout (≥ 1 month at scale in production, plus active in the wp.org plugin directory), no open critical issues, host adapters in use by more than one host (or the API is deemed ready regardless).

## v2 — full-sidebar personalization

**Gated by design sign-off + community signal.** v0.1 through v1.0 deliberately scope reorder to plugin-added items inside the curated Plugins group. Early usability feedback and the public launch thread independently asked for the whole sidebar to be personalizable, not just the Plugins group. v2 is where that scope expansion is evaluated and, if the design holds up, delivered.

**Direction:**

- Reorder or move any sidebar item, not only plugin-added ones.
- Broader ordering options beyond manual drag (for example alphabetical, or per-user custom; scoped vs. global is a design decision).
- A built-in, user-managed lower-priority area, without introducing arbitrary user-defined groups.
- Optional alternative sidebar placement.
...
**Exit criteria:** design sign-off on the full-sidebar interaction and reset model; the tracked work resolved or explicitly descoped; no regression to the default (unconfigured) sidebar, which still preserves core positions.

## Future — Make/Core proposal

**Gated by community signal.** If the plugin gains traction in the public WordPress space (real users beyond the original incubation environment), we open a Make/Core proposal extracting the core-portable parts:
...
- **A vertical-tabs / collapsed-core information architecture** (we won't restructure core into a different IA, and the default sidebar never moves or hides core items on the user's behalf). Note: *user-initiated* full-sidebar personalization is a different thing, and is no longer a non-goal. It is now tracked as the gated **v2** phase above.

If you want to argue any of these on, open a Discussion. We listen.

## Tracking

- **GitHub Milestones**: two coarse tracking buckets, not one milestone per phase. `v1` collects everything on the path to a stable 1.0 (the phases through v1.0 above); `v2` is the full-sidebar personalization work. Issues are filed against whichever bucket they belong to.
